...
**标准**：改源 → `./build.sh` → `./commit.sh -m "..."` → push（走当前登录的 gh CLI）。

**commit 身份**：这台机器的全局 git config 是工作邮箱（挂在另一个 GitHub 账号 `fanxiaolong123` 下），而仓库归 `fannnnner`。直接 `git commit` 会让这个仓库的 commit 在 GitHub 上挂到错误账号。所以**必须用 `./commit.sh`** 提交，它通过 `--author` 和 `GIT_COMMITTER_*` 环境变量一次性覆盖作者/提交者，不碰任何 git config。
